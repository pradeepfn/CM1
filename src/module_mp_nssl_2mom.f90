...
! uncomment next line to turn on timings for routine sections
!#define TIMINGS
...
! Autoconversion parameters
  ...
  real   , public  :: ccn            = 1.5e+09   ! set in namelist!! Central plains CCN value
...
! 0,2, 5.00e-10, 1, 0, 0, 0      : itype1,itype2,cimas0,icfn,ihrn,ibfc,iacr
  ...
  real   , public :: ehw0 = 1.0          ! constant or max assumed graupel-droplet collection efficiency
  real   , private :: erw0 = 1.0          ! constant assumed rain-droplet collection efficiency
  real   , public :: ehlw0 = 1.0         ! constant or max assumed hail-droplet collection efficiency
  ...
  real   , public :: dfrz = 0.25e-3  ! minimum diameter of frozen drops from Bigg freezing (used for vfrz) for iacr > 1
...
!
      ...
      parameter( xvhlmn=0.523599*(0.3e-3)**3, xvhlmx=0.523599*(40.e-3)**3 )  ! mks xvfmx = (pi/6)*(10mm)**3
...
SUBROUTINE nssl_2mom_driver(qv, qc, qr, qi, qs, qh, qhl, ccw, crw, cci, csw, chw, chl,  &
                              ...
                              scr,scw,sci,scs,sch,schl,sctot,noninduc,                  &  ! space charge  - mp_phys = 25
                              induc,elec,scion,sciona,                                  &  ! lightning - mp_phys = 26
                              ipelectmp,                                                &
                              diagflag,                                                 &
...
! for real cases when hydrometeor mixing ratios have been initialized without concentrations
     ...
     &                    t0,t7,infdo,jy,its,jts,timesed1,timesed2,timesed3,zmaxsed,timesetvt)
...
!
     ...
     &                    t0,t7,infdo,jslab,its,jts,timesed1,timesed2,timesed3,zmaxsed,timesetvt)
!
...
!              vent1 = (xdia(mgs,lr,1)**(-2. - alpr))*(1./xdia(mgs,lr,1) + 0.5*fx(lr))**(-2.5-alpr-0.5*bx(lr)) 
!              vent2 = (xdia(mgs,lr,1)*(1./xdia(mgs,lr,1) + 0.5*fx(lr))**(-0.5-0.5*bx(lr)) )**(-2.-alpr)
              
!              rwvent(mgs) =    &
!     &        ( 0.78*x +    &
!     &          0.308*fvent(mgs)*y*(xdia(mgs,lr,1)**(-2. - alpr))*   &
!     &            Sqrt(ax(lr)*rhovt(mgs))*(1./xdia(mgs,lr,1) + 0.5*fx(lr))**(-2.5-alpr-0.5*bx(lr)) )

!            rwvent(mgs) =    &
!     &       0.78*x +    &
!     &       0.308*fvent(mgs)*y*   &
!     &        Sqrt(ax(lr)*rhovt(mgs))*(xdia(mgs,lr,1)*(1./xdia(mgs,lr,1) + 0.5*fx(lr))**(-0.5-0.5*bx(lr)) )**(-2.-alpr)
        
!             IF ( .not. ( rwvent(mgs) > -1. .and. rwvent(mgs) < 1.e5) ) THEN
!               write(0,*) 'problem with rwvent in NUCOND: rwvent = ',rwvent(mgs)
!               write(0,*) 'vent1,vent2 = ',vent1,vent2
!               write(0,*) 'x,fvent,y,g1palp = ',x,fvent(mgs),y,g1palp
!               x = (1./xdia(mgs,lr,1) + 0.5*fx(lr))**(-2.5-alpr-0.5*bx(lr)) 
!               write(0,*) 'terms = ',x,alpr,0.5*bx(lr),1./xdia(mgs,lr,1) ,0.5*fx(lr)
!               write(0,*) 'arg = ',(xdia(mgs,lr,1)*(1./xdia(mgs,lr,1) + 0.5*fx(lr))**(-0.5-0.5*bx(lr)) )
!               write(0,*) 'xdia(mgs,lr,1),3 = ',xdia(mgs,lr,1),xdia(mgs,lr,3)
!               write(0,*) 'more terms = ',(xdia(mgs,lr,1)**(-2. - alpr))
!               write(0,*) 'term3 = ',(1./xdia(mgs,lr,1) + 0.5*fx(lr)),(1./xdia(mgs,lr,1) + 0.5*fx(lr))**(2.5+alpr+0.5*bx(lr))
!               write(0,*) '----------'
!             ENDIF
...
! Note: get here if there is no previous cloud water and w > 0.
      cn(mgs) = 0.0
!      IF ( ssmax(mgs) .lt. sscb .and. dcloud .gt. qxmin(lc)) THEN
      IF ( dcloud .gt. qxmin(lc)) THEN
...
! a few vars for time-split fallout      
      ...
      integer ix,jy,kz, il, ic, ir, icp1, irp1, ip1,jp1,kp1
      integer i,j
      real slope1, slope2
      real x1, x2, x3
      real eps,eps2
...
!      
      ...
      real ccnc(ngs),cina(ngs),ccna(ngs)
      real sscb  ! 'cloud base' SS threshold
      parameter ( sscb = 2.0 )
      integer idecss  ! flag to turn on (=1) decay of ssmax when no cloud or ice crystals
      parameter ( idecss = 1 )
...
!            IF ( tke(mgs) > 0. .and. my_rank == 0) write(0,*) 'ec0fac: ',    &
!!            IF ( my_rank == 0) write(0,*) 'ec0fac: ',    &
!     &       (3.94 + (5.46-3.94)/(0.15)*tke(mgs))/3.94,tke(mgs),ec0(mgs),    &
!     &        cracr(mgs),cx(mgs,lr),xdia(mgs,lr,3)*1.e3
!            ec0(mgs) = ec0(mgs)*(3.94 + (5.46-3.94)/(0.15)*tke(mgs))/3.94
            tmp = Min(1.5, (3.94 + (5.46-3.94)/(0.15)*tke(mgs))/3.94)
            cracr(mgs) = cracr(mgs)*tmp
            qracw(mgs) = qracw(mgs)*tmp
            cracw(mgs) = cracw(mgs)*tmp
          ENDIF
         ENDIF
        ENDIF
        ENDIF
...
!     &         Sqrt(ax(lr)*rhovt(mgs))*(xdia(mgs,lr,1)*(1./xdia(mgs,lr,1) + 0.5*fx(lr))**(-0.5-0.5*bx(lr)) )**(-2.-alpr)
             ...
             IF ( .not. ( rwvent(mgs) > -1. .and. rwvent(mgs) < 1.e5) ) THEN
               write(0,*) 'problem with rwvent in GS: rwvent = ',rwvent(mgs),mgs,ngscnt
               write(0,*) 'vent1,vent2,vent1/vent2 = ',vent1,vent2,vent1/vent2
               write(0,*) 'x,fvent,y,g1palp = ',x,fvent(mgs),y,g1palp
               write(0,*) 'fschm,fakvisc = ', fschm(mgs),fakvisc(mgs),fakvisc(mgs),fwvdf(mgs)
               write(0,*) 'temg,temcg,i,k ',temg(mgs),temcg(mgs),igs(mgs),kgs(mgs)
               vent1 = (1./xdia(mgs,lr,1) + 0.5*fx(lr))**(-2.5-alpr-0.5*bx(lr)) 
               write(0,*) 'terms = ',x,alpr,0.5*bx(lr),1./xdia(mgs,lr,1) ,0.5*fx(lr)
               write(0,*) 'arg = ',(xdia(mgs,lr,1)*(1./xdia(mgs,lr,1) + 0.5*fx(lr))**(-0.5-0.5*bx(lr)) )
               write(0,*) 'xdia(mgs,lr,1),3 = ',xdia(mgs,lr,1),xdia(mgs,lr,3)
               write(0,*) 'more terms = ',(xdia(mgs,lr,1)**(-2. - alpr))
               ...
               vent2 = dble(1./xdia(mgs,lr,1) + 0.5*fx(lr))**dble(2.5+alpr+0.5*bx(lr))
               write(0,*) 'term3 = ',(1./xdia(mgs,lr,1) + 0.5*fx(lr)),(1./xdia(mgs,lr,1) + 0.5*fx(lr))**(2.5+alpr+0.5*bx(lr))
               write(0,*) 'vent1/2 = ',vent1,vent2, 2.5+alpr+0.5*bx(lr)
               vent1 = dble(1.d0/dble(xdia(mgs,lr,1)))**(2. + alpr)
               write(0,*) 'vent1 = ,',vent1
               write(0,*) '----------'
               STOP
             ENDIF
